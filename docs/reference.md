...
The setup script installs PortAudio, PyAudio, the ElevenLabs Python SDK, QR matrix support, and the agent-browser 0.34.0 native binary. It creates runtime files under `~/.config/omarchy/omarvis/` and `~/.local/share/omarvis/`. It does not edit this plugin checkout. The shell script only talks to you and sequences the phases; every operation that touches a file, a secret, or the network runs in `omarvis/setupfiles.py` on the system Python (standard library only, 3.11 or newer).

### Reproducible install

Every artifact setup fetches is pinned in this repository, so the plugin commit under review fully determines the code that runs. No package-manager lifecycle script and no managed-browser installer ever runs:
...
- **agent-browser** — the npm package tarball `agent-browser-0.34.0.tgz` ships the native binary for every platform. Setup downloads that tarball from the npm registry over HTTPS with a size cap and deadline, verifies its sha256 (and the same digest npm records as `integrity`, both committed in `omarvis/setupfiles.py`), extracts only `bin/agent-browser-linux-<arch>` with a per-member size cap, verifies that binary's committed sha256, and installs it as `~/.local/share/omarvis/agent-browser/agent-browser`. npm and node are not used at all, so the package's `postinstall` script (which would fetch a binary from a GitHub release URL) never runs, and `agent-browser install` is never invoked: if no Chromium is present, setup offers the distribution package (`omarchy pkg add chromium`) instead.
- **ElevenLabs browser client** — the phone page's SDK (`@elevenlabs/client` 1.23.0 `dist/lib.iife.js`) is downloaded the same way from the npm registry; the tarball sha256 and the member sha256 are committed in `omarvis/setupfiles.py`, and on any mismatch nothing is installed.
- **System packages** — `portaudio`, `python-pyaudio`, and optionally `wtype` and `chromium` come from the distribution's signed repositories via `omarchy pkg add`; their versions follow the distribution, not this plugin.

### Setup safety

- **Secrets never enter argv.** The ElevenLabs key is validated and stored by the helper, which reads it from stdin and sends it as a request header over HTTPS with no redirects, a response cap, and a deadline. `curl` is not used.
- **Runtime files are written the way the daemon reads them.** `config.json`, `api_key`, `profile.md`, and `setup.log` are created through a directory descriptor with `O_NOFOLLOW`, validated by `fstat` (regular file, owned by you, one link, size-capped, private mode), and replaced atomically from an `O_EXCL` temporary file. A pre-planted symlink, FIFO, or hard link at any of those paths is refused, and both runtime directories are made 0700 before anything is written.
- **The bindings edit is a transaction.** Setup reads `~/.config/hypr/bindings.lua` through its directory descriptor, writes a timestamped backup, replaces the file atomically with its mode preserved, checks the syntax with `luac -p` when available, asks Hyprland to reload, and reads `hyprctl configerrors`. Any error restores the backup and reloads again.
- **A failed or interrupted run is undone.** Every change setup makes (previous config, bindings backup, whether the plugin was already enabled) is journaled first. If setup exits with an error the journal is replayed newest-first; if it crashes or loses power, the next run replays it before doing anything else.

### Runtime hardening

- **Helpers run from a validated descriptor.** `omarvis/process.py` resolves every helper (`omarchy`, `hyprctl`, `herdr`, `wl-copy`, `wtype`, `tailscale`, `agent-browser`) along a PATH whose directory chains must be root- or self-owned and not group/other writable, opens the file with `O_NOFOLLOW`, validates it by `fstat`, and executes it through `/proc/self/fd/N`, so the file that runs is exactly the file that was validated. Shell-script helpers run through their interpreter bound the same way, with the script's validated absolute path as its argument. Nothing Python runs in the child between fork and exec (`preexec_fn` is never used), which keeps spawning safe from every other thread in the daemon.
- **Helpers get a minimal environment.** Children never inherit the daemon's environment. Each gets a fixed allowlist of session variables (`HOME`, `PATH` filtered to trusted directories, `XDG_*`, `WAYLAND_DISPLAY`, `HYPRLAND_INSTANCE_SIGNATURE`, `DBUS_SESSION_BUS_ADDRESS`, locale) plus a short per-program list (`OMARCHY_*` for `omarchy`, `HERDR_*` for `herdr`, `AGENT_BROWSER_*` and the display hints for `agent-browser`). Loader and interpreter startup variables and unrelated credentials never reach a helper. Dictated text reaches `wl-copy` on stdin, never in argv.
- **Every child is bounded and tracked.** Output is streamed through fixed caps, the whole process group is signalled on timeout or overflow, and a supervisor terminates what is still running on session end, takeover, or shutdown.
- **The tailnet web service is bounded before authentication.** At most 16 concurrent connections are accepted (extra ones get a bare 503 and are closed before any header is parsed), every socket has a 15 s read/write deadline, request bodies are capped, at most 2 event streams exist at a time, each with a bounded backlog and a lifetime cap, and shutdown closes every open connection deterministically. Served assets are read once through descriptors and held in memory; request logs never include the query string.
...
`SUPER + CTRL + J` starts or ends the two-way voice call — the only kind of session Omarvis has. Hold `SUPER + J` to dictate your own words into the focused window; release to type them. While holding, press `SPACE` and let go within a second to go hands-free: the recording stays open, a fresh `SUPER + J` tap types it, and `ESCAPE` discards it. If you keep holding after `SPACE` and let go later, that release types it straight away. The window is `dictation.handsfree_release_grace_ms` (default 1000). Holding `SUPER + J` puts Hyprland into a small submap, so the chord never reaches the global `SUPER + SPACE` menu binding, and the submap ends with the release. Once you let go after the chord, the shell moves Hyprland into a second submap for as long as the recording stays open, where `SUPER + J` and `ESCAPE` are the only bindings; it ends the moment the recording does, or at shell startup. Left-click the bar microphone, or press `SUPER + ALT + J`, to open the Omarvis panel. Right-click is intentionally inert. Setup deletes any `SUPER + SHIFT + J` Ask binding or `SUPER + ALT + J` text binding an older install left behind, and rewrites a pre-submap dictation pair.
